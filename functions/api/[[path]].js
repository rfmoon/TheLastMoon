...
async function routeRequest(request, env, url) {
  ...
  if (
    url.pathname === "/api/xpay-checker/transactions" &&
    request.method === "GET"
  ) {
    requireMenuAccess(user, "xpay-checker");
    return listXpayCheckerTransactions(env.DB, url);
  }

  if (
    url.pathname === "/api/xpay-checker/transactions/bulk" &&
    request.method === "POST"
  ) {
    requireMenuAccess(user, "xpay-checker");
    return upsertXpayCheckerTransactions(request, env.DB, user);
  }
...
async function listXpayCheckerTransactions(db, url) {
  ...
  if (!isXpayIsoDate(from) || !isXpayIsoDate(to)) {
    throw new AppError(
      400,
      "date_from dan date_to wajib berformat YYYY-MM-DD.",
      "xpay-checker-date"
    );
  }
  ...
  const result = await db.prepare(`
    ...
    ORDER BY payment_date ASC, payment_sec ASC, id ASC
  `).bind(from, to).all();
...
async function upsertXpayCheckerTransactions(request, db, user) {
  const body = await readJson(request);
  const input = Array.isArray(body.rows) ? body.rows : [];
  ...
  if (!input.length) {
    throw new AppError(
      400,
      "Tidak ada transaksi XPay yang dikirim.",
      "xpay-checker-empty"
    );
  }
  ...
  if (input.length > 250) {
    throw new AppError(
      400,
      "Maksimal 250 transaksi per request.",
      "xpay-checker-batch-limit"
    );
  }
  ...
  for (const item of input) {
    const payment = String(item?.payment || "").trim();
    const parts = parseXpayPayment(payment);
    if (!parts) continue;
    ...
    const member = String(item?.member || "").trim().slice(0, 200);
    const partner = String(
      item?.partner || item?.partner_id || ""
    ).trim().slice(0, 240);
  ...
  if (!rows.length) {
    throw new AppError(
      422,
      "Tidak ada PAYMENT XPay valid untuk disimpan.",
      "xpay-checker-payment"
    );
  }

  const statements = rows.map(row =>
    db.prepare(`
      INSERT INTO xpay_transactions (
        signature,
        transaction_id,
        payment,
        payment_date,
...
function parseXpayPayment(value) {
...
function isXpayIsoDate(value) {
  return /^\d{4}-\d{2}-\d{2}$/.test(String(value || ""));
}

function xpayFiniteNumber(value) {
  const number = Number(value);
