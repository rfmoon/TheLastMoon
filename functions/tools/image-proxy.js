...
function isPrivateHost(hostname) {
  ...
  if (/^127\./.test(host) || /^0\./.test(host) || /^10\./.test(host)) {
    return true;
  }
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return jsonError(502, "Server Cloudflare tidak dapat mengambil gambar dari link tersebut.");
  }

  if (!upstream.ok) {
    return jsonError(
      502,
      `Server gambar membalas HTTP ${upstream.status}.`
    );
  }
  ...
  if (declaredLength > MAX_IMAGE_BYTES) {
    return jsonError(413, "Ukuran gambar terlalu besar. Maksimal 12 MB.");
  }

  const buffer = await upstream.arrayBuffer();

  if (!buffer.byteLength) {
    return jsonError(422, "File gambar kosong.");
  }
  ...
  if (!contentType) {
    return jsonError(415, "Link tersebut tidak menghasilkan file gambar.");
  }
...
export async function onRequest(context) {
  if (context.request.method !== "GET") {
    return jsonError(405, "Method tidak diizinkan.");
  }
